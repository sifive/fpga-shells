...
import sifive.fpgashells.ip.intel.{FIFO, ddr4_64bit}

class AltmemphyDDR4RAM(implicit p: Parameters) extends LazyModule {
  ...
  override lazy val module = new AltmemphyDDR4RAMImp(this)
}

class AltmemphyDDR4RAMImp(_outer: AltmemphyDDR4RAM)(implicit p: Parameters) extends LazyModuleImp(_outer) {
  val addrSize = log2Ceil(_outer.size / 16)

  val (in, edge) = _outer.node.in(0)
  val ddr4 = Module(new ddr4_64bit)
  require(ddr4.io.local_address.getWidth == addrSize)
  val tl_clock = clock
  val ddr_clock = ddr4.io.aux_full_rate_clk
  val mem_if = IO(new MemIfBundle)

  // tl_ prefixed hw are from CPU clock domain
  // ddr_ prefixed hw are from DDR4 controller clock domain
  ...
  withClock(ddr_clock) {
    ...
    ddr4.io.local_read_req := rreq
    ddr4.io.local_write_req := wreq

    ddr4.io.local_size := 1.U
    ddr4.io.local_burstbegin := true.B

    ddr4.io.local_address := ddr_req.address
    ddr4.io.local_be := ddr_req.be
    ddr4.io.local_wdata := ddr_req.wdata
    ...
    val will_read_request = !fifo_req.io.rdempty && !rreq && !wreq && !rreq_pending && ddr4.io.local_ready
    val will_respond = !fifo_resp.io.wrfull && ((rreq_pending && ddr4.io.local_rdata_valid) || (wreq && ddr4.io.local_ready))
    ...
    when (will_respond) { // the response will be sent next clock cycle
      ...
      wreq := false.B
      ddr_resp.rdata := ddr4.io.local_rdata
    }
    when (rreq && ddr4.io.local_ready) {
      rreq := false.B
    }
  }
  ...
  mem_if.connectFrom(ddr4.io)
}

trait HasAltmemphyDDR4 { this: BaseSubsystem =>
  val dtb: DTB
  val mem_ctrl = LazyModule(new AltmemphyDDR4RAM)
  mem_ctrl.node := mbus.toDRAMController(Some("altmemphy-ddr4"))()
}

trait HasAltmemphyDDR4Imp extends LazyModuleImp {
  val outer: HasAltmemphyDDR4
  val mem_if = IO(new MemIfBundle)
