...
package sifive.fpgashells.devices.xilinx.xilinxmig
...
import sifive.fpgashells.ip.xilinx.mig.{MIGIOClocksReset, MIGIODDR, mig}
...
class XilinxMIGPads(depth : BigInt, fpga: String) extends MIGIODDR(depth, fpga) {
  def this(c : XilinxMIGParams, f: String) {
    this(AddressRange.fromSets(c.address).head.size, f)
  }
  override def cloneType = {
    new XilinxMIGPads(depth, fpga).asInstanceOf[this.type]
  }
}

class XilinxMIGIO(depth : BigInt, fpga: String) extends MIGIODDR(depth, fpga) with MIGIOClocksReset

class XilinxMIGIsland(c : XilinxMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
...
//  require((depth<=0x10000000L),"artymig supports upto 256 MB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxMIGIO(depth, c.fpga)
    })
    ...
    val blackbox = Module(new mig(depth, c.fpga))
    ...
    if (c.fpga == "arty") blackbox.io.clk_ref_i.foreach(_ := io.port.clk_ref_i.get)
...
class XilinxMIG(c : XilinxMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxMIGIsland(c, crossing))
