...
// BlackBox definitions for AWS sh_ddr tiedown interface

// should rewrite to mimic vcu118mig instantiation
...
class XilinxF1VU9PDDR(c: XilinxF1VU9PDDRParams)(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxF1VU9PDDRPads
    })
...
class XilinxF1VU9PDDRPads extends F1VU9PDDRIO with F1VU9PDDRMgmt// with F1VU9PDDRAnalog
...
class XilinxF1VU9PDDRIsland(c: XilinxF1VU9PDDRParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val blackbox = Module(new F1VU9PDDRBlackBox(c.instantiate))
    ...
    attach(io.port.M_A_DQ,      blackbox.io.M_A_DQ)
    attach(io.port.M_A_ECC,     blackbox.io.M_A_ECC)
    attach(io.port.M_A_DQS_DP,  blackbox.io.M_A_DQS_DP)
    attach(io.port.M_A_DQS_DN,  blackbox.io.M_A_DQS_DN)
    attach(io.port.M_B_DQ,      blackbox.io.M_B_DQ)
    attach(io.port.M_B_ECC,     blackbox.io.M_B_ECC)
    attach(io.port.M_B_DQS_DP,  blackbox.io.M_B_DQS_DP)
    attach(io.port.M_B_DQS_DN,  blackbox.io.M_B_DQS_DN)
    attach(io.port.M_D_DQ,      blackbox.io.M_D_DQ)
    attach(io.port.M_D_ECC,     blackbox.io.M_D_ECC)
    attach(io.port.M_D_DQS_DP,  blackbox.io.M_D_DQS_DP)
    attach(io.port.M_D_DQS_DN,  blackbox.io.M_D_DQS_DN)*/
    // OUTPUTS
    // block A
...
/*    
    ...
    // AXI
    //--------------------------
    blackbox.io.cl_sh_ddr_awid    := Cat(axi.map(_.aw.bits.id))
    blackbox.io.cl_sh_ddr_awaddr  := Cat(axi.map(_.aw.bits.addr)) 
    blackbox.io.cl_sh_ddr_awlen   := Cat(axi.map(_.aw.bits.len))
    blackbox.io.cl_sh_ddr_awsize  := Cat(axi.map(_.aw.bits.size))
    blackbox.io.cl_sh_ddr_awburst := Cat(axi.map(_.aw.bits.burst))
    blackbox.io.cl_sh_ddr_awvalid := Cat(axi.map(_.aw.valid))
    Cat(axi.map(_.aw.ready))      := blackbox.io.sh_cl_ddr_awready
    
    // Amazon claims their IO is AXI-4 but Arm says otherwise; there is no WID in AXI-4
    // leave disconnected so Chisel ties it to 0
    //blackbox.io.cl_sh_ddr_wid     :=

    blackbox.io.cl_sh_ddr_wdata   := Cat(axi.map(_.w.bits.data))
    blackbox.io.cl_sh_ddr_wstrb   := Cat(axi.map(_.w.bits.strb))
    blackbox.io.cl_sh_ddr_wlast   := Cat(axi.map(_.w.bits.last))
    blackbox.io.cl_sh_ddr_wvalid  := Cat(axi.map(_.w.valid))
    Cat(axi.map(_.w.ready))       := blackbox.io.sh_cl_ddr_wready
    Cat(axi.map(_.b.bits.id))     := blackbox.io.sh_cl_ddr_bid      
    Cat(axi.map(_.b.bits.resp))   := blackbox.io.sh_cl_ddr_bresp    
    Cat(axi.map(_.b.valid))       := blackbox.io.sh_cl_ddr_bvalid   
    blackbox.io.cl_sh_ddr_bready  := Cat(axi.map(_.b.ready))
    blackbox.io.cl_sh_ddr_arid    := Cat(axi.map(_.ar.bits.id)) 
    blackbox.io.cl_sh_ddr_araddr  := Cat(axi.map(_.ar.bits.addr))
    blackbox.io.cl_sh_ddr_arlen   := Cat(axi.map(_.ar.bits.len))
    blackbox.io.cl_sh_ddr_arsize  := Cat(axi.map(_.ar.bits.size))
    blackbox.io.cl_sh_ddr_arburst := Cat(axi.map(_.ar.bits.burst))
    blackbox.io.cl_sh_ddr_arvalid := Cat(axi.map(_.ar.valid))
    Cat(axi.map(_.ar.ready))      := blackbox.io.sh_cl_ddr_arready  
    Cat(axi.map(_.r.bits.id))     := blackbox.io.sh_cl_ddr_rid      
    Cat(axi.map(_.r.bits.data))   := blackbox.io.sh_cl_ddr_rdata    
    Cat(axi.map(_.r.bits.resp))   := blackbox.io.sh_cl_ddr_rresp    
    Cat(axi.map(_.r.bits.last))   := blackbox.io.sh_cl_ddr_rlast    
    Cat(axi.map(_.r.valid))       := blackbox.io.sh_cl_ddr_rvalid   
    blackbox.io.cl_sh_ddr_rready  := Cat(axi.map(_.r.ready))
