...
trait PolarFireInitMonitorIOPads {
  val AUTOCALIB_DONE             = Bool(OUTPUT)
  val DEVICE_INIT_DONE           = Bool(OUTPUT)
  val FABRIC_POR_N               = Bool(OUTPUT)
  val PCIE_INIT_DONE             = Bool(OUTPUT)
  val SRAM_INIT_DONE             = Bool(OUTPUT)
  val SRAM_INIT_FROM_SNVM_DONE   = Bool(OUTPUT)
  val SRAM_INIT_FROM_SPI_DONE    = Bool(OUTPUT)
  val SRAM_INIT_FROM_UPROM_DONE  = Bool(OUTPUT)
  val USRAM_INIT_DONE            = Bool(OUTPUT)
  val USRAM_INIT_FROM_SNVM_DONE  = Bool(OUTPUT)
  val USRAM_INIT_FROM_SPI_DONE   = Bool(OUTPUT)
  val USRAM_INIT_FROM_UPROM_DONE = Bool(OUTPUT)
  val XCVR_INIT_DONE             = Bool(OUTPUT)
}
...
class PolarFireInitMonitor(implicit val p:Parameters) extends BlackBox
{
  ...
  ElaborationArtefacts.add(s"${desiredName}.libero.tcl",
    s"""create_design -id Actel:SgCore:PF_INIT_MONITOR:2.0.103 -design_name {${desiredName}} -config_file {} -params {} -inhibit_configurator 0
